...
export default function AdminTransactionsPage({ params }: AdminTransactionsPageProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetch transactions
        const fetchedTransactions = await getAdminTransactions(params.clinicId)
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <p className="text-xl text-red-500 mb-4">{error || "Error loading transactions"}</p>
          <button 
